...
+ (NSMutableArray *)mainColoursInImage:(UIImage *)image {
    ...
    // 默认
    CGFloat dimension = 10;
    CGFloat flexibility = 2;
    CGFloat range = 60;
...
//    range = 20;
    ...
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
    Byte *rawData = (Byte *) calloc(dimension * dimension * 4, sizeof(unsigned char));
    ...
    CGFloat flexFactor = flexibility * 2 + 1;
    CGFloat factor = flexFactor * flexFactor * 3; //(r,g,b) == *3
    for (int n = 0; n<(dimension * dimension); n++){
    ...
    for (NSString * key in temp){
        CGFloat count = [temp[key] floatValue];
        CGFloat percentage = count/totalCount;
        NSArray * rgb = [key componentsSeparatedByString:@","];
        CGFloat r = [rgb[0] floatValue];
        CGFloat g = [rgb[1] floatValue];
        CGFloat b = [rgb[2] floatValue];
        UIColor * colour = [UIColor colorWithRed:(r/255.0f) green:(g/255.0f) blue:(b/255.0f) alpha:1.0f];
        [arr addObject:colour];
