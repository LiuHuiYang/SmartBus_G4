...
- (BOOL)deleteCurrentTransformer:(SHCurrentTransformer *)currentTransformer {
    ...
    NSString *deleteSQL = [NSString stringWithFormat:@"delete from CurrentTransformer where CurrentTransformerID = %tu and ID = %tu;", currentTransformer.currentTransformerID, currentTransformer.id];
...
- (NSMutableArray *)getAllCurrentTransformers {
    ...
    NSString *currentTransformerSql = [NSString stringWithFormat:
        @"select ID, CurrentTransformerID, SubnetID, DeviceID,  \
        Remark, voltage, Channel1, Channel2, Channel3, Channel4,\
        ...
        Channel20, Channel21, Channel22, Channel23,Channel24   \
        from  CurrentTransformer order by CurrentTransformerID;"];
    
    NSMutableArray *array = [self selectProprty:currentTransformerSql];
    
    NSMutableArray *allCurrentTransformers = [NSMutableArray arrayWithCapacity:array.count];
    
    for (NSDictionary *dict in array) {
        
        [allCurrentTransformers addObject:( [[SHCurrentTransformer alloc] initWithDict:dict])];
    }
    
    return allCurrentTransformers;
}

/// 增加新的CurrentTransformer 返回 id
- (NSInteger)insertCurrentTransformer:(SHCurrentTransformer *)CurrentTransformer {
    
    NSString *insertSQL = [NSString stringWithFormat:@"insert into CurrentTransformer (CurrentTransformerID) values (%tu);", CurrentTransformer.currentTransformerID];
    ...
    if (res) {
        ...
        // 获得ID号
        NSString *string = [NSString stringWithFormat:@"select max(ID) from CurrentTransformer;"];
...
- (BOOL)updateCurrentTransformer:(SHCurrentTransformer *)currentTransformer {
    ...
    NSString *updateSQL = [NSString stringWithFormat:
        ...
        @"update CurrentTransformer set Remark = '%@',         \
        SubnetID = %d, DeviceID = %d, voltage = %tu,           \
        Channel1  = '%@', Channel2  = '%@',  Channel3 = '%@',    \
        Channel4  = '%@', Channel5  = '%@',  Channel6 = '%@',  \
        Channel7  = '%@', Channel8  = '%@',  Channel9 = '%@',  \
        Channel10 = '%@', Channel11 = '%@', Channel12 = '%@',  \
        Channel13 = '%@', Channel14 = '%@', Channel15 = '%@',  \
        Channel16 = '%@', Channel17 = '%@', Channel18 = '%@',  \
        Channel19 = '%@', Channel20 = '%@', Channel21 = '%@',  \
        Channel22 = '%@', Channel23 = '%@', Channel24 = '%@'   \
            where CurrentTransformerID = %tu;",
                           
                           currentTransformer.remark,
                           currentTransformer.subnetID,
                           currentTransformer.deviceID,
                           currentTransformer.voltage,
                           
                           currentTransformer.channel1,
                           currentTransformer.channel2,
                           currentTransformer.channel3,
                           currentTransformer.channel4,
                           currentTransformer.channel5,
                           currentTransformer.channel6,
                           currentTransformer.channel7,
                           currentTransformer.channel8,
                           currentTransformer.channel9,
                           currentTransformer.channel10,
                           currentTransformer.channel11,
                           currentTransformer.channel12,
                           currentTransformer.channel13,
                           currentTransformer.channel14,
                           currentTransformer.channel15,
                           currentTransformer.channel16,
                           currentTransformer.channel17,
                           currentTransformer.channel18,
                           currentTransformer.channel19,
                           currentTransformer.channel20,
                           currentTransformer.channel21,
                           currentTransformer.channel22,
                           currentTransformer.channel23,
                           currentTransformer.channel24,
...
- (NSInteger)getMaxCurrentTransformerID {
    ...
    NSString *sql = [NSString stringWithFormat:@"select max(CurrentTransformerID) from CurrentTransformer;"];
    
    id resID = [[[self selectProprty:sql] lastObject] objectForKey:@"max(CurrentTransformerID)"];
...
- (BOOL)updateCentralMacroCommand:(SHMacroCommand *)command {
    ...
    NSString *updateSQL = [NSString stringWithFormat:
                           @"update MacroButtonCommands set Remark = '%@',    \
                           SubnetID = %d, DeviceID = %d, CommandTypeID = %tu, \
                           FirstParameter = %tu, SecondParameter = %tu,       \
                           ThirdParameter = %tu,                              \
                           DelayMillisecondAfterSend = %tu where              \
                           MacroID = %tu and ID = %tu;",
                           command.remark, command.subnetID, command.deviceID,
                           command.commandTypeID, command.firstParameter,
                           command.secondParameter, command.thirdParameter,
                           command.delayMillisecondAfterSend, command.macroID,
                           command.id
                           ];
...
- (NSInteger)insertNewCentralMacroCommand:(SHMacroCommand *)command {
    ...
    NSString *insertSQL = [NSString stringWithFormat:
                           @"insert into MacroButtonCommands (MacroID,      \
                           Remark, SubnetID, DeviceID, CommandTypeID,       \
                           FirstParameter, SecondParameter, ThirdParameter, \
                           DelayMillisecondAfterSend) values                \
                           (%tu, '%@', %d, %d, %tu, %tu, %tu, %tu, %tu);",
                           
                           command.macroID, command.remark,
                           command.subnetID,command.deviceID,
                           command.commandTypeID, command.firstParameter,
                           command.secondParameter, command.thirdParameter,
                           command.delayMillisecondAfterSend
    ...
    if (res) {
        ...
        NSString *string = [NSString stringWithFormat:@"select max(ID) from MacroButtonCommands;"];
...
- (NSMutableArray *)getCentralMacroCommands:(SHMacro *)macro {
    ...
    NSString *macroButtonCommandSql = [NSString stringWithFormat:
                                       @"select ID, MacroID, Remark,        \
                                       SubnetID, DeviceID, CommandTypeID,   \
                                       FirstParameter, SecondParameter,     \
                                       ThirdParameter,                      \
                                       DelayMillisecondAfterSend from       \
                                       MacroButtonCommands                  \
                                       where MacroID = %tu order by id;",
                                       
                                       macro.macroID
                                       ];
    
    NSMutableArray *array = [self selectProprty:macroButtonCommandSql];
    
    NSMutableArray *centeralLightCommands =
        [NSMutableArray arrayWithCapacity:array.count];
    
    for (NSDictionary *dict in array) {
        
        SHMacroCommand *command =
            [[SHMacroCommand alloc] initWithDict:dict];
        
        [centeralLightCommands addObject: command];
    }
    
    return centeralLightCommands;
}

/// 获取所有的宏命令按钮
- (NSMutableArray *)getAllCentralMacros {
    
    NSString *lightSql = [NSString stringWithFormat:@"select id, MacroID, MacroName, MacroIconName from MacroButtons order by MacroID;"];
    
    NSMutableArray *array = [self selectProprty:lightSql];
    
    NSMutableArray *centeralMacroButtons = [NSMutableArray arrayWithCapacity:array.count];
    
    for (NSDictionary *dict in array) {
        
        SHMacro *macro =
            [[SHMacro alloc] initWithDict:dict];
        
        [centeralMacroButtons addObject:macro];
    }
    
    return centeralMacroButtons;
}

/// 删除Macro
- (BOOL)deleteMacro:(SHMacro *)macro {
    
    NSString *deleteCommandSQL = [NSString stringWithFormat:@"delete from MacroButtonCommands where MacroID = %tu;", macro.macroID];
    
    [self executeSql:deleteCommandSQL];
    
    NSString *deleteSQL = [NSString stringWithFormat:@"delete from MacroButtons where MacroID = %tu;", macro.macroID];
...
- (BOOL)insertNewMacro:(SHMacro *)macro {
    ...
    NSString *insertSql = [NSString stringWithFormat:
                           @"insert into MacroButtons (MacroID, MacroName,  \
                            MacroIconName) values(%tu, '%@', '%@');",
...
- (BOOL)updateMacro:(SHMacro *)macro {
    ...
    NSString *updateSQL = [NSString stringWithFormat:
                           @"update MacroButtons set MacroName = '%@',  \
                           MacroIconName = '%@' where MacroID = %tu;",
                           
                           macro.macroName, macro.macroIconName, macro.macroID
                           ];
...
- (NSUInteger)getMaxMacroID {
    ...
    // 获得结果ID
    id resID = [[[self selectProprty:@"select max(MacroID) from MacroButtons"] lastObject] objectForKey:@"max(MacroID)"];
...
- (NSMutableArray *)getCentralLightCommands:(SHCentralLight *)light {
    ...
    NSString *lightCommandSql = [NSString stringWithFormat:@"select id, FloorID, CommandID, Remark, SubnetID, DeviceID, CommandTypeID, Parameter1, Parameter2, DelayMillisecondAfterSend from CentralLightsCommands where FloorID = %tu order by id;", light.floorID];
    
    NSMutableArray *array = [self selectProprty:lightCommandSql];
    
    NSMutableArray *centeralLightCommands = [NSMutableArray arrayWithCapacity:array.count];
    
    for (NSDictionary *dict in array) {
        
        [centeralLightCommands addObject:[[SHCentralLightCommand alloc] initWithDict:dict]
        ];
    }
...
- (NSMutableArray *)getAllCentralLights {
    ...
    NSString *lightSql = [NSString stringWithFormat:@"select id, FloorID, FloorName from CentralLights order by FloorID;"];
    
    NSMutableArray *array = [self selectProprty:lightSql];
    
    NSMutableArray *centeralLights = [NSMutableArray arrayWithCapacity:array.count];
    
    for (NSDictionary *dict in array) {
        [centeralLights addObject:
            ([[SHCentralLight alloc] initWithDict:dict])
        ];
    }
    
    return centeralLights;
}
...
- (BOOL)deleteSecurity:(SHSecurityZone *)securityZone {
    ...
    // 1.先删除指令
    NSString *deleteSQL = [NSString stringWithFormat:@"delete from CentralSecurity where ID = %tu;", securityZone.id];
...
- (BOOL)insertNewSecurity:(SHSecurityZone *)securityZone {
    ...
    NSString *insertSql = [NSString stringWithFormat:@"insert into CentralSecurity (ID, SubnetID, DeviceID, ZoneID, zoneNameOfSecurity) values(%tu, %d, %d, %tu, '%@');", securityZone.id, securityZone.subnetID, securityZone.deviceID, securityZone.zoneID, securityZone.zoneNameOfSecurity];
...
- (BOOL)updateSecurityZone:(SHSecurityZone *)securityZone {
    ...
    NSString *updateSQL = [NSString stringWithFormat:@"update CentralSecurity set zoneNameOfSecurity = '%@', SubnetID = %d, DeviceID = %d, ZoneID = %tu where ID = %tu;", securityZone.zoneNameOfSecurity, securityZone.subnetID, securityZone.deviceID, securityZone.zoneID, securityZone.id];
...
- (NSUInteger)getMaxSecurityID {
    ...
    // 获得结果ID
    id resID = [[[self selectProprty:@"select max(ID) from CentralSecurity"] lastObject] objectForKey:@"max(ID)"];
...
- (NSMutableArray *)getAllCentralSecuity {
    ...
    NSString *secuitySql = [NSString stringWithFormat:@"select id, SubnetID, DeviceID, ZoneID, zoneNameOfSecurity from CentralSecurity;"];
    
    NSArray *array = [self selectProprty:secuitySql];
    
    NSMutableArray *secuityies = [NSMutableArray arrayWithCapacity:array.count];
    
    for (NSDictionary *dict in array) {
        
        [secuityies addObject:[[SHSecurityZone alloc] initWithDict:dict]];
    }
...
- (NSMutableArray *)getCentralHVACCommands:(SHCentralHVAC *)hvac {
    ...
    NSString *hvacCommandSql = [NSString stringWithFormat:
                                @"select id, FloorID, CommandID, Remark, SubnetID, DeviceID, CommandTypeID, Parameter1, Parameter2, DelayMillisecondAfterSend from CentralHVACCommands where FloorID = %tu order by id;", hvac.floorID];
    
    NSMutableArray *array = [self selectProprty:hvacCommandSql];
    
    NSMutableArray *hvacCommands = [NSMutableArray arrayWithCapacity:array.count];
    
    for (NSDictionary *dict in array) {
    
        [hvacCommands addObject:
            [[SHCentralHVACCommand alloc] initWithDict:dict]
        ];
    }
...
- (NSMutableArray *)getAllCentralHVACs {
    ...
    NSString *hvacSql = [NSString stringWithFormat:@"select id, FloorID, FloorName, isHaveHot from CentralHVAC order by FloorID;"];
    
    NSMutableArray *array = [self selectProprty:hvacSql];
    
    NSMutableArray *centeralHvacs = [NSMutableArray arrayWithCapacity:array.count];
    
    for (NSDictionary *dict in array) {
         
        [centeralHvacs addObject:([[SHCentralHVAC alloc] initWithDict:dict])];
    }
    
    return centeralHvacs;
}
...
- (void)alertTablesOrColumnName {
    ...
    // 增加CT24的电压设定值
    if (![self isColumnName:@"Voltage" consistinTable:@"CurrentTransformer"]) {
        
        [self executeSql:@"ALTER TABLE CurrentTransformer ADD Voltage INTEGER NOT NULL DEFAULT 0;"];
    }
    
    if (![self isColumnName:@"Channel1" consistinTable:@"CurrentTransformer"]) {
        
        for (Byte i = 1; i <= 24; i++) {
            
            NSString *sql = [NSString stringWithFormat:@"ALTER TABLE CurrentTransformer ADD Channel%@ TEXT NOT NULL DEFAULT 'CH%@'", @(i), @(i)];
